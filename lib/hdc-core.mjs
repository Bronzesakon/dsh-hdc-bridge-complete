...
export function createHdcCore(deps) {
  const { runShellRaw, psQuote, detectShellFlavor, getShellFlavor, setShellFlavor, fsService, log = console.log } = deps
  let hdcPath = null
  let hdcError = ''
  let retrying = null
  const diagLog = []
  ...
  function candidateList() {
    ...
    const seen = new Set()
    const roots = []
    for (const found of studio.discoverHdcCandidates().candidates) list.push(found.path)
    for (const found of studio.discoverStudioRoots().roots) roots.push(found.root + '\\sdk')
    for (const root of HDC_ROOTS) roots.push(root)
