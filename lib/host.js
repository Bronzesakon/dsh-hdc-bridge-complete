...
export function apply(ctx) {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const cmd = shellFlavor === 'pwsh'
      ? 'Set-Location -LiteralPath ' + psQuote(base) + '; $env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + '; & ' + psQuote(hvigor.path) + ' ' + args
      : 'cd ' + psQuote(base) + ' && DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + ' ' + psQuote(hvigor.path) + ' ' + args
    const run = async (runPolicy) => runShellRaw(cmd, timeoutMs, 1048576, runPolicy, base)
    let r = await run(policy)
    let out = (r.stdout && r.stdout.text) || ''
    let err = (r.stderr && r.stderr.text) || ''
    let retriedHostPolicy = false
    const cacheFailure = /(?:ENOENT[^\r\n]*(?:hvigor\.js|project_caches)|(?:hvigor\.js|project_caches)[^\r\n]*ENOENT|EPERM[^\r\n]*(?:\.hvigor|project_caches|node_modules))/i.test(out + '\n' + err)
    ...
    return { available: true, ok: buildOk({ exitCode: r.exitCode, timedOut: r.timedOut === true, stdout: out, stderr: err }), exitCode: r.exitCode, timedOut: r.timedOut === true, retriedHostPolicy, output: tailText((out + '\n' + err).trim(), 8000) }
  }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'list_devices') {
        ...
        return { ok: targets.ok, action, deviceCount: devices.length, lineCount: null, devices, targets: targets.targets, text, error: targets.error || '' }
      }
