  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'collect') {
        ...
        const cli = await ensureCli(policy)
        if (cli.ok) {
          // devecocli log applies --tail BEFORE any local filtering; when a pid/bundle filter is requested, fetch a wider tail so the filter has enough material to work on.
          const cliTail = (bundle || pidArg) ? Math.max(lines, 5000) : lines
          const argv = compileCli.buildDevecoCliLogArgs({ device: args.device_id, keyword, tail: cliTail })
