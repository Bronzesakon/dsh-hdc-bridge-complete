...
export function apply(ctx) {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const projectKey = String(base).replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '').slice(-48) || 'project'
    const isolatedHome = joinPath(tmpdir(), 'dsh-hvigor-' + projectKey)
    const commandFor = (home) => shellFlavor === 'pwsh'
      ? 'Set-Location -LiteralPath ' + psQuote(base) + '; $env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + (home ? '; $env:HVIGOR_USER_HOME=' + psQuote(home) + '; New-Item -ItemType Directory -Force -Path ' + psQuote(home) + ' | Out-Null' : '') + '; & ' + psQuote(hvigor.path) + ' ' + args
      : (home ? 'mkdir -p ' + psQuote(home) + ' && ' : '') + 'cd ' + psQuote(base) + ' && DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + (home ? ' HVIGOR_USER_HOME=' + psQuote(home) : '') + ' ' + psQuote(hvigor.path) + ' ' + args
    ...
    return { available: true, ok: buildOk({ exitCode: r.exitCode, timedOut: r.timedOut === true, stdout: out, stderr: err }), exitCode: r.exitCode, timedOut: r.timedOut === true, retriedHostPolicy, retriedIsolatedHome, hvigorUserHome: retriedIsolatedHome ? isolatedHome : '', output: tailText((out + '\n' + err).trim(), 8000) }
  }
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          if (buildOk(r) && !deMojo) {
            return { ...outside, backend: 'devecocli', ok: true, exitCode: deExit, timedOut: r.timedOut, output: deOutput }
          }
        ...
        if (fb.available && fb.ok) {
          return { ...outside, backend: 'hvigorw', ok: true, exitCode: fb.exitCode, timedOut: fb.timedOut, output: fb.output, note2: (deMojo ? 'devecocli channel was mojo-blocked in this sandbox (0x5) — fell back to the direct hvigorw build. ' : 'devecocli failed — used the direct hvigorw build. ') + 'hvigorw uses the signingConfigs already present in build-profile.json5; unsigned builds cannot install on a device.' }
        }
      ...
      if (args.action === 'run') {
        ...
        if (cli.ok) {
          ...
          if (buildOk(r) && !mojo) {
            return { ...outside, backend: 'devecocli', ok: true, exitCode: r.exitCode, timedOut: r.timedOut, output: tailText((r.stdout + '\n' + r.stderr).trim(), 8000) }
          }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (cli.ok) {
        ...
        ok = buildOk(result) && !mojo
      }
      const hapAfter = await findNewestHap(cwd, policy)
      const afterM = statMtimeMs(hapAfter)
      const artifactStale = args.clean !== true && ok && afterM <= beforeM
      if (artifactStale) ok = false

      let backend = cli.ok ? 'devecocli' : 'none'
      let fallback = false
      let fallbackText = ''
      if (!ok && args.clean !== true) {
        fallback = true
        const direct = await hvigorDirectBuild({ base: cwd, product: String(args.product || 'default'), mode: String(args.build_mode || 'debug'), timeoutMs: 30 * 60 * 1000, policy })
        if (direct.available && direct.ok) {
          const fallbackHap = await findNewestHap(cwd, policy)
          if (statMtimeMs(fallbackHap) > beforeM) {
            ok = true
            backend = 'hvigorw'
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const command = (shellFlavor === 'pwsh' ? '& ' : '') + psQuote(nodeBin) + ' ' + argv.join(' ')
      ...
      const errorCount = typeof (parsed && parsed.summary && parsed.summary.errorCount) === 'number' ? parsed.summary.errorCount : errors.length
      ...
      if (action === 'list_devices') {
        ...
        if (cli.ok) {
          ...
          if (result.exitCode === 0 && !mojoFatal(result) && rows.length > 0) {
            return { ok: true, action, deviceCount: rows.length, lineCount: null, devices: rows, text: output }
          }
        }
        await ensureHdc(policy)
        if (!hdcPathRef()) {
