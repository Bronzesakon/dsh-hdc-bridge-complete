...
export function apply(ctx) {
  ...
  async function hdcFallbackDeploy(args, cwd, policy) {
    ...
    const unsignedNote = /unsigned/i.test(hap)
      ? (inst.ok
        ? '\n  (unsigned artifact — fine on emulators; a physical device needs signing config in build-profile.json5)'
        : '\n  (unsigned artifact rejected — configure signing in build-profile.json5, or use hms_build sign)')
      : ''
    const text = [
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy }) {
    const st = getStudio()
    const sdk = getSdk()
    const hvigor = st.ok ? studio.hvigorwPath(st.root) : { ok: false, path: '', kind: '' }
    if (!hvigor.ok || !sdk.ok) {
      return { available: false, error: 'Fallback needs a DevEco Studio install: ' + (hvigor.ok ? '' : 'hvigorw not found. ') + (sdk.ok ? '' : 'SDK not found.') }
    }
    const cmd = shellFlavor === 'pwsh'
    ...
    const out = (r.stdout && r.stdout.text) || ''
    const err = (r.stderr && r.stderr.text) || ''
    return { available: true, ok: buildOk({ exitCode: r.exitCode, timedOut: r.timedOut === true, stdout: out, stderr: err }), exitCode: r.exitCode, timedOut: r.timedOut === true, output: tailText((out + '\n' + err).trim(), 8000) }
  }

  registerTool({
    name: 'hms_build',
    description: 'Build/sign/run a HarmonyOS project through the official DevEco CLI (devecocli) when available. When devecocli is unavailable OR its channel is blocked by the sandbox (mojo platform-channel 0x5), build/run automatically fall back to the local hvigorw direct build and the hdc_install/hdc_app loop — no caller action needed. Actions: status, build, run, sign, clean. Signing requires a one-time browser OAuth (devecocli auth login) plus a connected device; sign verifies build-profile.json5 actually changed and reports honest failure otherwise. Build/run/sign are [Outside sandbox] operations per the official docs.',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          deMojo = mojoFatal(r)
          deExit = r.exitCode
          deOutput = tailText((r.stdout + '\n' + r.stderr).trim(), 8000)
          if (buildOk(r) && !deMojo) {
            return { ...outside, backend: 'devecocli', ok: true, exitCode: deExit, timedOut: r.timedOut, mojoFatal: false, output: deOutput }
          }
        ...
        if (fb.available && fb.ok) {
          return { ...outside, backend: 'hvigorw', ok: true, exitCode: fb.exitCode, timedOut: fb.timedOut, mojoFatal: deMojo, output: fb.output, note2: (deMojo ? 'devecocli channel was mojo-blocked in this sandbox (0x5) — fell back to the direct hvigorw build. ' : 'devecocli failed — used the direct hvigorw build. ') + 'hvigorw uses the signingConfigs already present in build-profile.json5; unsigned builds cannot install on a device.' }
        }
        return { ...outside, backend: 'hvigorw', ok: false, exitCode: fb.available ? fb.exitCode : deExit, timedOut: fb.timedOut === true, mojoFatal: deMojo, output: (deOutput ? deOutput + '\n\n' : '') + (fb.available ? 'hvigorw fallback also failed:\n' + fb.output : 'hvigorw fallback unavailable: ' + fb.error), error: fb.available ? 'hvigorw build failed' : fb.error }
      }
      if (args.action === 'run') {
        let deRunNote = ''
        if (cli.ok) {
          ...
          if (buildOk(r) && !mojo) {
            return { ...outside, backend: 'devecocli', ok: true, exitCode: r.exitCode, timedOut: r.timedOut, mojoFatal: false, output: tailText((r.stdout + '\n' + r.stderr).trim(), 8000) }
          }
          deRunNote = mojo ? 'devecocli run channel was mojo-blocked in this sandbox (0x5) — using the fallback loop. ' : 'devecocli run failed — using the fallback loop. '
        }
        ...
        const fb = await hvigorDirectBuild({ base, product: q(args.product) || 'default', mode: q(args.buildMode) || 'debug', timeoutMs, policy })
        if (!fb.available || !fb.ok) return { ...outside, backend: 'hvigorw', ok: false, stage: 'build', output: fb.available ? fb.output : '', error: fb.available ? 'hvigorw build failed' : fb.error }
        ...
        if (bundleName) {
          ...
          return { ...outside, backend: 'hvigorw', ok: started.ok, stage: 'start', hap, bundleName, start: { ok: started.ok, hint: started.hint }, note: deRunNote + 'Fallback loop: hvigorw build -> hdc install -> aa start.' }
        }
        return { ...outside, backend: 'hvigorw', ok: true, stage: 'installed', hap, note: deRunNote + 'Installed; bundleName could not be read from AppScope/app.json5 — start it with hdc_app action=start bundleName=<from AppScope/app.json5>.' }
      }
      if (args.action === 'sign') {
        ...
        // artifact-verification pattern as build_project (#1). A silent
        // auth status would also make the logged-in check pass vacuously.
        const base = q(args.projectPath) || policyRoot(policy) || (typeof process !== 'undefined' && typeof process.cwd === 'function' ? process.cwd() : '')
        ...
        if (mojoFatal(status)) {
          return { ...outside, backend: 'devecocli', ok: false, stage: 'auth', error: 'devecocli mojo platform channel blocked in this sandbox (0x5) — sign needs [Outside sandbox] execution', mojoFatal: true }
        }
        ...
        const error = ok ? ''
          ...
          : (empty ? 'devecocli signature generate returned no output — the mojo channel likely died before signing ran (same family as build_project). Sign in DevEco Studio (Project Structure > Signing Configs) or run devecocli auth login + signature generate in a host terminal.'
          ...
          : (r.stderr || r.stdout))))
        return { ...outside, backend: 'devecocli', ok, exitCode: r.exitCode, mojoFatal: mojo, emptyOutput: empty, profileChanged, output: tailText(out, 6000), error, hint: ok ? 'signingConfigs/products were written to build-profile.json5; then hms_build action=build.' : '' }
      }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'check') {
        ...
        const mojo = mojoFatal(r)
        const ok = !r.timedOut && !mojo && (r.exitCode === 0 || LINT_REPORT_RE.test(r.stdout + r.stderr))
        return { outsideSandbox: false, backend: 'devecocli', ok, exitCode: r.exitCode, timedOut: r.timedOut, mojoFatal: mojo, format: jsonOk ? 'json' : 'text', result: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — check lint needs [Outside sandbox] execution' : (r.stderr || r.stdout)) }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      // devecocli keeps its docs index under the user profile
      // (~/.local/share/deveco-cli/docs or DEVECO_CLI_DATA_DIR), which the
      // session sandbox policy can block — the CLI then exits 0 with no
      // output at all. Retry once under the host default policy (read-only
      // docs access, same spirit as the [Outside sandbox] compile ops).
      const runDocs = async (argv, opts) => {
        let r = await runCli(cli, argv, opts, policy)
        if (r.exitCode === 0 && !r.timedOut && !mojoFatal(r) && !(r.stdout + r.stderr).trim()) {
          r = await runCli(cli, argv, opts, undefined)
        }
        return r
      }
      const DOCS_EMPTY_ERROR = 'devecocli docs returned no output — its docs index (under the user profile) is unreadable in this sandbox even after retrying under the host default policy. Run with [Outside sandbox] permissions, or use the online official docs.'
      if (args.action === 'catalog') {
        const r = await runDocs(['docs', 'catalog'], { timeoutMs: 60000, stdoutMaxBytes: 262144 })
        const mojo = mojoFatal(r)
        const empty = !(r.stdout + r.stderr).trim()
        const ok = r.ok && !mojo && !empty
        return { ok, output: tailText((r.stdout + '\n' + r.stderr).trim(), 6000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — docs needs [Outside sandbox] execution' : (empty ? DOCS_EMPTY_ERROR : (r.stderr || r.stdout))), fallback: ok ? '' : 'Online official docs: developer.huawei.com (use web search tools with the target API version in the query).', mojoFatal: mojo, emptyOutput: empty }
      }
      if (args.action === 'search') {
        const kws = String(args.keywords || '').split(/\s+/).map((s) => s.trim()).filter(Boolean)
        ...
        const r = await runDocs(argv, { timeoutMs: 60000, stdoutMaxBytes: 524288 })
        const mojo = mojoFatal(r)
        const empty = !(r.stdout + r.stderr).trim()
        const ok = r.ok && !mojo && !empty
        const json = tryParseJson(r.stdout.trim())
        return { ok, backend: 'devecocli', results: ok ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 6000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — docs needs [Outside sandbox] execution' : (empty ? DOCS_EMPTY_ERROR : (r.stderr || r.stdout))), fallback: ok ? '' : 'Online official docs: developer.huawei.com (use web search tools with the target API version in the query).', mojoFatal: mojo, emptyOutput: empty, hint: 'Search matches ANY keyword (tokenized); for a precise topic use a longer phrase or --catalog. Use hms_docs action=read with a documentId from the results to get full content.' }
      }
      if (args.action === 'read') {
        const id = String(args.documentId || '').trim()
        if (!id) return { ok: false, error: 'documentId is required for action=read' }
        const r = await runDocs(['docs', 'read', psQuote(id)], { timeoutMs: 60000, stdoutMaxBytes: 524288 })
        const mojo = mojoFatal(r)
        const empty = !(r.stdout + r.stderr).trim()
        const ok = r.ok && !mojo && !empty
        return { ok, documentId: id, content: tailText((r.stdout + '\n' + r.stderr).trim(), 12000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — docs needs [Outside sandbox] execution' : (empty ? DOCS_EMPTY_ERROR : (r.stderr || r.stdout))), fallback: ok ? '' : 'Online official docs: developer.huawei.com (use web search tools with the target API version in the query).', mojoFatal: mojo, emptyOutput: empty }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'versions') {
        ...
        return { ok, versions: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 4000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — check compat needs [Outside sandbox] execution' : (r.stderr || r.stdout)), mojoFatal: mojo, hint: upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge.' : '' }
      }
      if (args.action === 'diff') {
        ...
        const ok = r.ok && !mojo
        const upgradeNeeded = /required component is missing|minimum required version/i.test(r.stderr + r.stdout)
        return { outsideSandbox: true, note: OUTSIDE_NOTE, ok, exitCode: r.exitCode, timedOut: r.timedOut, mojoFatal: mojo, sourceVersion: src, targetVersion: tgt, changes: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — check compat needs [Outside sandbox] execution' : (r.stderr || r.stdout)), hint: upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge.' : '' }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
  ...
  registerTool({
    name: 'build_project',
    description: 'Compile and package a HarmonyOS project or modules with devecocli (DevEco CLI). When devecocli fails or its channel is blocked by the sandbox (mojo 0x5), it automatically falls back to the direct hvigorw build (backend=hvigorw) and verifies a new .hap artifact — no caller action needed. Defaults when omitted: --product default, --build-mode debug. Run switch_cwd first if the session directory is not the HarmonyOS project root. Output is truncated to the last 50 lines and always ends with a BUILD FAILED / Build completed successfully status line.',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args, exec) {
      ...
      // Snapshot the newest .hap before the build so a "success" that produced
      // no new artifact can be detected (devecocli can report BUILD SUCCESSFUL
      // while the hvigor child never really ran — see ScribePad issue #1).
      ...
      const mojo = mojoFatal(r)
      let ok = buildOk(r)
      const hapAfter = await findNewestHap(cwd, policy)
      const afterM = statMtimeMs(hapAfter)
      const artifactStale = ok && (afterM <= beforeM)
      if (artifactStale) ok = false
      // Automatic fallback: the devecocli channel is blocked or produced no new
      // artifact in this sandbox — run the direct hvigorw build before
      // reporting failure (hvigorw needs no mojo IPC and is verified to work
      // here; see ScribePad 鸿蒙插件问题记录.md #1).
      let backend = 'devecocli'
      let fallback = false
      let fallbackText = ''
      if (!ok) {
        fallback = true
        const fb = await hvigorDirectBuild({ base: cwd, product: String(args.product || 'default'), mode: String(args.build_mode || 'debug'), timeoutMs: 30 * 60 * 1000, policy })
        if (fb.available && fb.ok) {
          const fbHap = await findNewestHap(cwd, policy)
          const fbM = statMtimeMs(fbHap)
          if (fbM > beforeM) {
            ok = true
            backend = 'hvigorw'
            fallbackText = '\n\n✅ devecocli channel was blocked or produced no artifact in this sandbox — fell back to the direct hvigorw build (backend=hvigorw). New artifact: ' + (fbHap || '') + '\n' + fb.output
          } else {
            fallbackText = '\n\nhvigorw fallback reported success but no .hap was updated either (artifact mtime unchanged).\n' + fb.output
          }
        } else {
          fallbackText = fb.available ? '\n\nhvigorw fallback also failed:\n' + fb.output : '\n\nhvigorw fallback unavailable: ' + fb.error
        }
      }
      ...
      let text = cut.text
      if (ok && r.exitCode !== 0 && backend === 'devecocli') {
        text = text.replace(/BUILD FAILED \(exitCode=\d+\)/, `Build completed successfully (exitCode=${r.exitCode}, judged by output markers)`)
      }
      if (artifactStale && !ok) {
        text += '\n\n❌ Build reported success but no .hap was updated — the devecocli channel likely crashed before hvigor ran (mojo 0x5 or EPERM in a restricted sandbox). Last artifact: ' + (hapAfter || 'none') + '.'
      }
      if (mojo && !ok) {
        text += '\n\n❌ ' + MOJO_NOTE
      }
      ...
      return {
        ...
        note: OUTSIDE_NOTE,
        mojoFatal: mojo,
        artifactStale: artifactStale && !ok,
        hapBefore: hapBefore || '',
        hapAfter: hapAfter || '',
        warning: (ok && mojo) || (artifactStale && !ok) ? 'build markers passed but the artifact did not change — verify the .hap timestamp before installing' : '',
        text,
        error: ok ? '' : (mojo ? 'build failed — devecocli mojo platform channel blocked in this sandbox (0x5); the hvigorw fallback also failed' : (artifactStale ? 'build reported success but no .hap artifact was updated; the hvigorw fallback also failed' : 'build failed')),
      }
    },
  })
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const finalText = fallback
        ? (fallback.ok
          ? (fallback.text + (text
            ? '\n\n[devecocli discovery background — ' + (discoveryMojo ? 'mojo platform channel blocked (0x5)' : 'no device reported by devecocli') + ']\n' + text
            : ''))
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'collect') {
        ...
        let raw = ''
        let cliNote = ''
        // pid/bundle filtering needs the FULL buffer (post-hoc client-side
        // filtering), which devecocli log cannot guarantee: its --tail runs
        // before any filter and the mojo channel is often blocked in
        // sandboxes. For those queries, go straight to hdc shell hilog.
        const needFullBuffer = Boolean(bundle || pidArg)
        if (!needFullBuffer) {
          const cli = await ensureCli(policy)
          if (cli.ok) {
          ...
          cliNote = 'pid/bundle filter requires the full buffer — used hdc shell hilog'
        }
        if (!raw) {
          backend = 'hdc'
          await ensureHdc(policy)
          ...
          const hr = await runHdc(['shell', 'hilog', '-x'], { target: cur.target, timeoutMs: 60000, stdoutMaxBytes: 2097152 }, policy)
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const opts = { timeoutMs: (action === 'start' ? 15 : 5) * 60 * 1000, stdoutMaxBytes: 524288, workdir: cwd }
      // devecocli's emulator subcommand talks to the host via the same mojo
      // platform channel that the docs subcommand uses; in a restricted
      // sandbox it can exit 0 with no output at all (the profile-level
      // emulator registry is outside the workspace). Retry once under the
      // host default policy, mirroring the docs/listing retry.
      let r = await runCli(cli, cliArgs.map((a) => psQuote(a)), opts, policy)
      const emptyFirst = !(r.stdout + r.stderr).trim()
      if (r.exitCode === 0 && !r.timedOut && !mojoFatal(r) && emptyFirst) {
        r = await runCli(cli, cliArgs.map((a) => psQuote(a)), opts, undefined)
      }
      const out = compileOut.stripAnsi((r.stdout + '\n' + r.stderr).trim())
      const mojo = mojoFatal(r)
      const empty = !out
      const rows = action === 'status' || action === 'list' ? compileCli.parseCliTable(r.stdout) : []
      // Cross-check against hdc on every list/status, even when devecocli
      // returned nothing (mojo block): hdc is the authority on live links and
      // the acceptance criterion is "connected emulator listed OR hdcConnected".
      let hdcConnected = []
      if (action === 'list' || action === 'status') {
        try {
          const list = await listTargets(policy)
          const connected = list.targets.filter((t) => /connected/i.test(t.state)).map((t) => t.id)
          hdcConnected = connected
          for (const row of rows) {
            const serial = String(row.serial || '').trim()
            if (serial && serial !== '-' && connected.includes(serial)) row.hdcConnected = true
          }
        } catch { /* best effort */ }
      }
      const rowsForName = name ? rows.filter((row) => String(row.name || '').toLowerCase().includes(name.toLowerCase())) : rows
      const ok = r.exitCode === 0 && !mojo && !empty
      const statusText = rowsForName.length
        ...
        : (name
          ? `No emulator named "${name}" in the list output.`
          : (hdcConnected.length
            ? 'devecocli emulator list returned no rows (mojo channel blocked in this sandbox), but hdc sees connected target(s): ' + hdcConnected.join(', ')
            : 'No emulator rows parsed.'))
      const text = (action === 'status' || (action === 'list' && (mojo || empty)) ? statusText : out) || out || ''
      const hdcNote = hdcConnected.length
        ? ' hdc sees connected target(s): ' + hdcConnected.join(', ') + ' — a running emulator may list as "stopped" here (template state); trust hdc_list_targets for the live link.'
        : ''
      const note = mojo
        ? (MOJO_NOTE + hdcNote)
        : (empty
          ? 'devecocli emulator returned no output — the emulator registry (user profile) is unreadable in this sandbox even after retrying under the host default policy.' + hdcNote + ' Use hdc_list_targets for connected devices.'
          : (action === 'start' && ok
      ...
      const result = {
        ...
        note,
        text,
        error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — run with [Outside sandbox] permissions' : (empty ? 'devecocli emulator returned no output — sandbox cannot read the emulator registry; use hdc_list_targets' : 'emulator command failed')),
      }
      if (name) result.name = name
      if (action === 'list') result.rows = rows
      else if (action === 'status') result.rows = rowsForName
      if (action === 'list' || action === 'status') result.hdcConnected = hdcConnected
      return result
    },
  })
