...
export function apply(ctx) {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const envPrefix = (cacheRoot) => shellFlavor === 'pwsh'
      ? '$env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + '; ' + (cacheRoot ? '$env:HVIGOR_USER_HOME=' + psQuote(cacheRoot) + '; $env:BUILD_CACHE_DIR=' + psQuote(joinPath(cacheRoot, 'cache-env')) + '; ' : '') + javaPrefix
      : 'DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + ' ' + (cacheRoot ? 'HVIGOR_USER_HOME=' + psQuote(cacheRoot) + ' BUILD_CACHE_DIR=' + psQuote(joinPath(cacheRoot, 'cache-env')) + ' ' : '') + javaPrefix
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const pollEmulatorRunning = async (name, waitMs) => {
        ...
        let beforeIds = []
        try { beforeIds = (await listTargets(policy)).targets.map((t) => t.id) } catch { /* hdc may not be ready yet */ }
        while (Date.now() < deadline) {
          try {
            const list = await listTargets(policy)
            const hit = list.targets.find((t) => /connected/i.test(t.state) && isEmulatorTarget(t.id) && (beforeIds.length === 0 || !beforeIds.includes(t.id)))
          ...
          try {
            ...
            if (Array.isArray(instances)) {
              const hit = instances.find((i) => String(i.name || '').trim() === name && (i.isRunning === true || String(i.isRunning).toLowerCase() === 'true'))
      ...
      if (action === 'start') {
        ...
        return { ok: false, ...res, running: false, target: poll.target, verifiedBy: poll.method || '', error: 'devecocli start returned success but no running emulator appeared in hdc list targets within ' + Math.round((Math.min(timeoutMs, 180000)) / 1000) + 's' }
      } else if (action === 'stop') {
