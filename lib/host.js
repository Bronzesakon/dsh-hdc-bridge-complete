...
export function apply(ctx) {
  ...
  async function install(args, policy) {
    ...
    const hap = shellFlavor === 'pwsh' ? hapRaw.replace(/\//g, '\\') : hapRaw
  ...
  async function connect(args, policy) {
    ...
    const ok = r.ok && !/fail|error/i.test(out) && (/connect ok/i.test(out) || repeat || out === '')
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'list') {
        ...
        return { ok: r.exitCode === 0 && json != null, backend: 'devecocli', instances: json, output: json != null ? '' : tailText(((r.stdout || '') + (r.stderr || '')).trim(), 4000), error: r.exitCode !== 0 ? tailText((r.stderr || r.stdout || '').trim(), 1500) : (empty ? 'devecocli emulator list returned empty output — CLI may need auth (devecocli auth login) or a newer version.' : 'emulator list output was not valid JSON: ' + tailText(((r.stdout || '') + (r.stderr || '')).trim(), 300)), hint: epermHint((r.stderr || '') + (r.stdout || '')) }
      }
      if (action === 'start') {
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'versions') {
        ...
        return { ok: r.ok, versions: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 4000), error: r.ok ? '' : (r.stderr || r.stdout), hint: (upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge. ' : '') + epermHint((r.stderr || '') + (r.stdout || '')) }
      }
      if (args.action === 'diff') {
        ...
        const json = tryParseJson(r.stdout.trim())
        const upgradeNeeded = /required component is missing|minimum required version/i.test(r.stderr + r.stdout)
        return { ok: r.ok, exitCode: r.exitCode, timedOut: r.timedOut, sourceVersion: src, targetVersion: tgt, changes: json != null ? json : null, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: r.ok ? '' : (r.stderr || r.stdout), hint: (upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge. ' : '') + epermHint((r.stderr || '') + (r.stdout || '')) }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
