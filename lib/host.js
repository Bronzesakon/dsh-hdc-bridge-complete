...
export function apply(ctx) {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const envPrefix = (cacheRoot) => shellFlavor === 'pwsh'
      ? '$env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + '; ' + (cacheRoot ? '$env:HVIGOR_USER_HOME=' + psQuote(cacheRoot) + '; ' : '') + javaPrefix
      : 'DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + ' ' + (cacheRoot ? 'HVIGOR_USER_HOME=' + psQuote(cacheRoot) + ' ' : '') + javaPrefix
    ...
    const commandFor = (cacheRoot) => {
      ...
      return shellFlavor === 'pwsh'
        ? 'Set-Location -LiteralPath ' + psQuote(base) + '; ' + (cacheRoot ? 'New-Item -ItemType Directory -Force -Path ' + psQuote(cacheRoot) + ' | Out-Null; ' : '') + envPrefix(cacheRoot) + stopDaemon + '; ' + envPrefix(cacheRoot) + taskCommand
        : (cacheRoot ? 'mkdir -p ' + psQuote(cacheRoot) + ' && ' : '') + 'cd ' + psQuote(base) + ' && ' + envPrefix(cacheRoot) + stopDaemon + '; ' + envPrefix(cacheRoot) + taskCommand
    }
    ...
    return { available: true, ok: buildOk({ exitCode: r.exitCode, timedOut: r.timedOut === true, stdout: out, stderr: err }), exitCode: r.exitCode, timedOut: r.timedOut === true, retriedHostPolicy, retriedIsolatedHome, daemonStopped: true, javaHome, hvigorCacheDir: retriedIsolatedHome ? isolatedHome : '', output: tailText((out + '\n' + err).trim(), 8000) }
  }
