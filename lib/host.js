  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'sign') {
        ...
        const mojo = mojoFatal(r)
        const ok = r.ok && !mojo
        return { ...outside, backend: 'devecocli', ok, exitCode: r.exitCode, mojoFatal: mojo, output: tailText((r.stdout + '\n' + r.stderr).trim(), 6000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — sign needs [Outside sandbox] execution' : (r.stderr || r.stdout)), hint: ok ? 'On success, signingConfigs/products were written to build-profile.json5; then hms_build action=build.' : '' }
      }
