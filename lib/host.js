...
export function apply(ctx) {
    ...
    async execute(args, exec) {
      ...
      if (errors.length === 0) {
        return {
          ...
          text: `ArkTS Check Passed — no errors found in ${files.length} file(s).`,
        }
      }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const ok = results.some(({ command, result }) => String(command).startsWith('devecocli run ') && result && result.exitCode === 0)
      return {
        ok,
  ...
  registerTool({
    name: 'hdc_log',
    description: 'Collect, clear, or list HarmonyOS device logs. collect: fetch recent device logs filtered by a keyword/prefix (devecocli log); clear: clear the device log buffer (hdc shell hilog -r); list_devices: list connected devices (devecocli device list). Use for runtime evidence while debugging HarmonyOS apps.',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args, exec) {
      ...
      if (action === 'list_devices') {
        ...
        if (!output) {
          return { ok: true, action, deviceCount: 0, lineCount: undefined, text: 'No connected devices detected.' }
        }
        return { ok: r.exitCode === 0, action, deviceCount: rows.length > 0 ? rows.length : undefined, lineCount: undefined, text: output }
      }
      ...
      if (action === 'collect') {
        ...
        const argv = compileCli.buildDevecoCliLogArgs({ device: args.device_id, keyword: args.log_prefix, tail: lines })
        const r = await runCli(cli, argv.map((a) => psQuote(a)), { timeoutMs: 120000, stdoutMaxBytes: 1048576, workdir: cwd }, policy)
        if (r.exitCode !== 0 && !r.timedOut) {
          return { ok: false, exitCode: r.exitCode, text: `devecocli ${argv.join(' ')} failed (code=${r.exitCode}): ${r.stderr || r.stdout}` }
        }
        const logLines = compileOut.stripAnsi(r.stdout).split(/\r?\n/).map((s) => s.trim()).filter(Boolean)
        if (logLines.length === 0) {
          return {
            ok: true,
            action,
            device: String(args.device_id || 'default'),
            prefix: String(args.log_prefix || ''),
            lineCount: 0,
            text: ['No matching logs found.', `device: ${args.device_id || 'default'}`, `prefix: ${args.log_prefix || ''}`].join('\n'),
          }
        }
        return {
          ok: true,
          ...
          text: ['Log collection successful.', `device: ${args.device_id || 'default'}`, `prefix: ${args.log_prefix || ''}`, `count: ${logLines.length}`, '', '--- Log Content ---', ...logLines].join('\n'),
        }
      }
