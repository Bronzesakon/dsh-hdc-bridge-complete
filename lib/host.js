  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'list') {
        ...
        if (json == null) {
          const plain = await runCli(cli, ['emulator', 'list'], { timeoutMs: 60000, stdoutMaxBytes: 524288 }, policy)
        ...
        const output = text || (Array.isArray(json) && json.length ? JSON.stringify(json, null, 2) : 'No emulator instances found.')
        const ok = r.exitCode === 0 && !r.timedOut && !mojoFatal(r) && (json != null || !!text)
        ...
        return { ok, backend: 'devecocli', instances: json, hdcConnected, output: tailText(output, 8000), error }
      }
      if (action === 'start') {
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'versions') {
        ...
        if (json == null) {
          const plain = await runCli(cli, ['check', 'compat', 'versions'], { timeoutMs: 60000, stdoutMaxBytes: 262144 }, policy)
          const plainText = (plain.stdout + '\n' + plain.stderr).trim()
          if (plainText) {
            r = { ...plain, stdout: plainText, stderr: '' }
            text = plainText
      ...
      if (args.action === 'diff') {
        ...
        if (json == null) {
          ...
          const plain = await runCli(cli, plainArgv, { timeoutMs: Number(args.timeoutMs) || 600000, stdoutMaxBytes: 1048576 }, policy)
          const plainText = (plain.stdout + '\n' + plain.stderr).trim()
          if (plainText) {
            r = { ...plain, stdout: plainText, stderr: '' }
            text = plainText
