...
export function apply(ctx) {
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      // Cross-check against hdc: mark rows whose serial matches a connected
      // hdc target. TCP emulators may show a '-' serial while hdc sees them
      // at 127.0.0.1:5555 — note that in the output instead of guessing.
      let hdcConnected = []
      if ((action === 'list' || action === 'status') && rows.length) {
        try {
          const list = await listTargets(policy)
      ...
      const note = mojo
        ? MOJO_NOTE
        : (empty
          ? 'devecocli emulator returned no output — the emulator registry (user profile) is unreadable in this sandbox even after retrying under the host default policy. Use hdc_list_targets for connected devices.'
          : (action === 'start' && ok
      ...
      if (hdcConnected.length) result.hdcConnected = hdcConnected
      return result
    },
  })
