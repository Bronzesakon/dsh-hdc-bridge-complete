...
export function apply(ctx) {
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          const ok = buildOk(r) && !mojo
          return { ...outside, backend: 'devecocli', ok, exitCode: r.exitCode, timedOut: r.timedOut, mojoFatal: mojo, output: tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — build needs [Outside sandbox] execution' : (r.stderr || r.stdout)), hint: mojo ? MOJO_ARTIFACT_NOTE : '' }
        }
        const hvigor = st.ok ? studio.hvigorwPath(st.root) : { ok: false, path: '', kind: '' }
        if (!hvigor.ok || !sdk.ok) {
      ...
      if (args.action === 'run') {
        if (cli.ok) {
          ...
          const mojo = mojoFatal(r)
          const ok = buildOk(r) && !mojo
          return { ...outside, backend: 'devecocli', ok, exitCode: r.exitCode, timedOut: r.timedOut, mojoFatal: mojo, output: tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: ok ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — run needs [Outside sandbox] execution' : (r.stderr || r.stdout)) }
        }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (artifactStale) {
        text += '\n\n❌ Build reported success but no .hap was updated — the devecocli channel likely crashed before hvigor ran (mojo 0x5 or EPERM in a restricted sandbox). Last artifact: ' + (hapAfter || 'none') + '. Escalate the session policy, or build directly with hvigorw (see E:\\ScribePad\\鸿蒙插件问题记录.md #1 for the exact command).'
      }
      if (mojo) {
      ...
      return {
        ...
        hapBefore: hapBefore || '',
        hapAfter: hapAfter || '',
        warning: (ok && mojo) || artifactStale ? 'build markers passed but the artifact did not change — verify the .hap timestamp before installing' : '',
        text,
        error: ok ? '' : (mojo ? 'build failed — devecocli mojo platform channel blocked in this sandbox (0x5); escalate the session policy or run outside the sandbox' : (artifactStale ? 'build reported success but no .hap artifact was updated' : 'build failed')),
      }
    },
  })
