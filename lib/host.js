...
export function apply(ctx) {
  ...
  async function hdcFallbackDeploy(args, cwd, policy) {
    ...
    const installResult = await runHdc(['install', '-r', psQuote(hap)], { target: targetId, timeoutMs: 180000, stdoutMaxBytes: 65536 }, policy)
    ...
    if (pkg.ok) {
      try {
        ...
        if (r.exitCode === 0) {
          ...
          logInfo('[hdc-bridge] devecocli ' + cliCache.version + ' via optionalDependency')
        } else {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const javaHome = studioJavaHome(st.root)
    ...
    const commandFor = (cacheRoot) => shellFlavor === 'pwsh'
      ? 'Set-Location -LiteralPath ' + psQuote(base) + '; $env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + '; ' + javaPrefix + (cacheRoot ? 'New-Item -ItemType Directory -Force -Path ' + psQuote(cacheRoot) + ' | Out-Null; ' : '') + '& ' + psQuote(hvigor.path) + ' ' + (cacheRoot ? psQuote('build-cache-dir=' + cacheRoot) + ' ' : '') + args
      : (cacheRoot ? 'mkdir -p ' + psQuote(cacheRoot) + ' && ' : '') + 'cd ' + psQuote(base) + ' && DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + ' ' + javaPrefix + (cacheRoot ? psQuote('build-cache-dir=' + cacheRoot) + ' ' : '') + psQuote(hvigor.path) + ' ' + args
    const run = async (runPolicy, home) => runShellRaw(commandFor(home), timeoutMs, 1048576, runPolicy, base)
    ...
    return { available: true, ok: buildOk({ exitCode: r.exitCode, timedOut: r.timedOut === true, stdout: out, stderr: err }), exitCode: r.exitCode, timedOut: r.timedOut === true, retriedHostPolicy, retriedIsolatedHome, hvigorCacheDir: retriedIsolatedHome ? isolatedHome : '', output: tailText((out + '\n' + err).trim(), 8000) }
  }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (!cli.ok) {
        return { ok: false, backend: 'devecocli', error: cli.error, hint: '安装官方 CLI 后即可用模拟器控制（电量/GPS/传感器/摇一摇/折叠/运动场景）：npm i -g @deveco/deveco-cli（需 DevEco Studio ≥ 6.1.0）。' }
      }
      ...
      if (action === 'list') {
        ...
        return { ok: r.exitCode === 0 && json != null, backend: 'devecocli', instances: json, hdcConnected, output: json != null ? '' : tailText(((r.stdout || '') + (r.stderr || '')).trim(), 4000), error: r.exitCode !== 0 ? tailText((r.stderr || r.stdout || '').trim(), 1500) + hdcNote : (empty ? 'devecocli emulator list returned empty output — CLI may need auth (devecocli auth login) or a newer version.' + hdcNote : 'emulator list output was not valid JSON: ' + tailText(((r.stdout || '') + (r.stderr || '')).trim(), 300) + hdcNote) }
      }
      if (action === 'start') {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          if (buildResultOk(r) && !deMojo) {
            ...
            if (artifactVerified) {
              return { ...outside, backend: 'devecocli', ok: true, exitCode: deExit, timedOut: r.timedOut, output: deOutput, artifactVerified, hapBefore: hapBefore || '', hapAfter: hapAfter || '' }
            }
        ...
        if (fb.available && fb.ok) {
          ...
          const artifactVerified = args.action === 'clean' ? !hapAfter : Boolean(hapAfter)
          if (artifactVerified) {
            return { ...outside, backend: 'hvigorw', ok: true, exitCode: fb.exitCode, timedOut: fb.timedOut, output: fb.output, artifactVerified, hapBefore: hapBefore || '', hapAfter: hapAfter || '', note2: (deMojo ? 'devecocli channel was mojo-blocked in this sandbox (0x5) — fell back to the direct hvigorw build. ' : 'devecocli failed — used the direct hvigorw build. ') + 'hvigorw uses the signingConfigs already present in build-profile.json5; unsigned builds cannot install on a device.' }
          }
          return { ...outside, backend: 'hvigorw', ok: false, exitCode: fb.exitCode, timedOut: fb.timedOut, output: fb.output, error: args.action === 'clean' ? 'hvigorw reported success but build outputs were not cleaned' : 'hvigorw reported success but no .hap artifact was produced', hapBefore: hapBefore || '', hapAfter: hapAfter || '' }
        }
        return { ...outside, backend: 'hvigorw', ok: false, exitCode: fb.available ? fb.exitCode : deExit, timedOut: fb.timedOut === true, output: (deOutput ? deOutput + '\n\n' : '') + (fb.available ? 'hvigorw fallback also failed:\n' + fb.output : 'hvigorw fallback unavailable: ' + fb.error), error: fb.available ? 'hvigorw build failed' : fb.error }
      }
      if (args.action === 'run') {
        const base = q(args.projectPath) || policyRoot(policy) || (typeof process !== 'undefined' && typeof process.cwd === 'function' ? process.cwd() : '')
        let deRunNote = ''
        ...
        await ensureHdc(policy)
        if (!hdcPathRef()) return { ...outside, backend: 'hvigorw', ok: false, stage: 'install', hap, error: hdcErrorRef() }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'check') {
        ...
        const ok = !r.timedOut && !mojo && !missingJson && (r.exitCode === 0 || LINT_REPORT_RE.test(r.stdout + r.stderr))
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'versions') {
        ...
        const json = tryParseJson(r.stdout.trim())
        const mojo = mojoFatal(r)
        const emptyResult = json == null
        const upgradeNeeded = /required component is missing|minimum required version/i.test(r.stderr + r.stdout)
        return { ok: r.ok && !mojo && !emptyResult, versions: json != null ? json : null, mojoFatal: mojo, emptyResult, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 4000), error: r.ok && !mojo && !emptyResult ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — compat versions needs [Outside sandbox] execution' : (emptyResult ? 'devecocli compat versions returned no JSON result.' : (r.stderr || r.stdout))), hint: upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge.' : '' }
      }
      if (args.action === 'diff') {
        ...
        const emptyResult = json == null
        const upgradeNeeded = /required component is missing|minimum required version/i.test(r.stderr + r.stdout)
        return { ok: r.ok && !mojo && !emptyResult, exitCode: r.exitCode, timedOut: r.timedOut, sourceVersion: src, targetVersion: tgt, changes: json != null ? json : null, mojoFatal: mojo, emptyResult, output: json != null ? '' : tailText((r.stdout + '\n' + r.stderr).trim(), 8000), error: r.ok && !mojo && !emptyResult ? '' : (mojo ? 'devecocli mojo platform channel blocked in this sandbox (0x5) — compat diff needs [Outside sandbox] execution' : (emptyResult ? 'devecocli compat diff returned no JSON result.' : (r.stderr || r.stdout))), hint: upgradeNeeded ? 'check compat requires the newer DevEco Studio version named in the error (upgrade at developer.huawei.com). Until then, hms_api @since/@deprecated tags give per-API version knowledge.' : '' }
      }
      return { ok: false, error: 'unknown action: ' + args.action }
    },
  })
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const artifactStale = ok && (args.clean === true ? Boolean(hapAfter) : !hapAfter || afterM <= beforeM)
      ...
      if (!ok) {
        fallback = true
        const direct = await hvigorDirectBuild({ base: cwd, product: String(args.product || 'default'), mode: String(args.build_mode || 'debug'), timeoutMs: 30 * 60 * 1000, policy, task: args.clean === true ? 'clean' : 'assembleHap' })
        if (direct.available && direct.ok) {
          const fallbackHap = await findNewestHap(cwd, policy)
          const fallbackVerified = args.clean === true ? !fallbackHap : Boolean(fallbackHap) && statMtimeMs(fallbackHap) > beforeM
          hapAfter = fallbackHap
          if (fallbackVerified) {
            ok = true
            backend = 'hvigorw'
            fallbackText = args.clean === true
              ? '\n\nDirect hvigorw clean fallback completed. Build outputs were removed.\n' + direct.output
              : '\n\nDirect hvigorw fallback completed. New artifact: ' + (fallbackHap || '') + '\n' + direct.output
          } else {
            fallbackText = args.clean === true
              ? '\n\nhvigorw reported success but build outputs were not removed.\n' + direct.output
              : '\n\nhvigorw reported success but no .hap artifact was updated.\n' + direct.output
          }
        } else {
          fallbackText = direct.available ? '\n\nhvigorw fallback failed:\n' + direct.output : '\n\nhvigorw fallback unavailable: ' + direct.error
        }
      }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (!studioInfo.ok) throw new Error('DevEco Studio install not found; arkts_check needs the SDK ets-loader.')
      const nodeBin = studioNodeBin(studioInfo.root)
      if (!nodeBin) throw new Error('Node binary not found in DevEco Studio: ' + joinPath(studioInfo.root, 'tools', 'node'))
      ...
      if (!stdout) throw new Error(stderr || `arkts-check exited with code ${result.exitCode} but produced no output`)
      let parsed
      try {
      ...
      if (parsed && parsed.error && (!Array.isArray(parsed.errors) || parsed.errors.length === 0)) throw new Error(parsed.error)
      ...
      if (action === 'list_devices') {
        ...
        if (cli.ok) {
          ...
          if (result.exitCode === 0 && !mojoFatal(result) && rows.length > 0) {
            await ensureHdc(policy)
            const discovered = hdcPathRef() ? await listTargets(policy) : { ok: false, targets: [], error: '' }
