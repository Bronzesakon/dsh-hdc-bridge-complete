...
export function apply(ctx) {
  ...
  async function collectEtsFiles(cwd, policy) {
    ...
    const r = await runShellRaw(cmd, 60000, 262144, policy, cwd)
    const t = ((r.stdout && r.stdout.text) || '').trim()
    return t ? t.split(/\r?\n/).map((s) => s.trim()).filter(Boolean) : []
  }

  // Find the newest built .hap under the project root (used by the start_app
  // hdc fallback when devecocli discovery is unavailable).
  async function findNewestHap(cwd, policy) {
    ...
    const r = await runShellRaw(cmd, 60000, 32768, policy, cwd)
    return ((r.stdout && r.stdout.text) || '').trim()
  }
  ...
  async function hdcFallbackDeploy(args, cwd, policy) {
    ...
    if (!hap) {
      return {
        ok: false,
        error: 'hdc fallback found a device but no built .hap under ' + cwd + ' — run build_project first.',
    ...
    async execute(args, exec) {
      ...
      if (files.length === 0) {
        ...
        if (files.length === 0) {
          return {
            ok: false,
            error: 'Auto-collect found 0 .ets files under entry/src/main/ets. Check that switch_cwd points at the HarmonyOS project root (with an entry module), or pass an explicit files list.',
            fileCount: 0,
