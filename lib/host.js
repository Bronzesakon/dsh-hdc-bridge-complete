  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      if (args.action === 'build' || args.action === 'clean') {
        ...
        if (cli.ok) {
          ...
          if (buildResultOk(r) && !deMojo) {
            ...
            if (artifactVerified) {
              return { ...outside, backend: 'devecocli', ok: true, exitCode: deExit, timedOut: r.timedOut, output: compileOut.stripAnsi(deOutput), artifactVerified, hapBefore: hapBefore || '', hapAfter: hapAfter || '' }
            }
        ...
        if (fb.available && fb.ok) {
          ...
          return { ...outside, backend: 'hvigorw', ok: false, exitCode: fb.exitCode, timedOut: fb.timedOut, output: compileOut.stripAnsi(fb.output), error: args.action === 'clean' ? 'hvigorw reported success but build outputs were not cleaned' : 'hvigorw reported success but no .hap artifact was produced', hapBefore: hapBefore || '', hapAfter: hapAfter || '' }
        }
      ...
      if (args.action === 'run') {
        ...
        if (!fb.available || !fb.ok) return { ...outside, backend: 'hvigorw', ok: false, stage: 'build', output: fb.available ? compileOut.stripAnsi(fb.output) : '', error: fb.available ? 'hvigorw build failed' : fb.error }
        ...
        if (bundleName) {
          ...
          return { ...outside, backend: 'hvigorw', ok: started.ok && mission.ok, stage: 'start', hap, bundleName, target: current.target, start: { ok: started.ok, hint: started.hint }, missionVerified: Boolean(mission.ok), note: deRunNote + 'Fallback loop: hvigorw build -> hdc install -> aa start.' }
        }
        return { ...outside, backend: 'hvigorw', ok: false, stage: 'installed', hap, missionVerified: false, error: 'Installed, but bundleName could not be read from AppScope/app.json5; app was not started.', note: deRunNote + 'Installed; bundleName could not be read from AppScope/app.json5 — start it with hdc_app action=start bundleName=<from AppScope/app.json5>.' }
      }
      if (args.action === 'sign') {
    ...
    async execute(args, exec) {
      ...
      const text = fallback
        ...
        : (hdcAvailable.length > 0
          ? ['请指定要使用的设备。', '当前 hdc 在线设备：', '', ...hdcAvailable.map((target) => `  - ${target.id} (${target.type || 'device'})`), '', '请使用 hvd 参数指定设备名称，例如：', `- hvd="${hdcAvailable[0].id}"`, ...(cliText ? ['', '[devecocli background]', cliText] : [])].join('\n')
          : cliText)
      return {
        ok,
        mode,
