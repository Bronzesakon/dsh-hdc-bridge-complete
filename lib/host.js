...
export function apply(ctx) {
  ...
  async function hvigorDirectBuild({ base, product, mode, timeoutMs, policy, task = 'assembleHap' }) {
    ...
    const commandFor = (cacheRoot) => shellFlavor === 'pwsh'
      ? 'Set-Location -LiteralPath ' + psQuote(base) + '; $env:DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + '; $env:DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + (cacheRoot ? '; New-Item -ItemType Directory -Force -Path ' + psQuote(cacheRoot) + ' | Out-Null' : '') + '; & ' + psQuote(hvigor.path) + ' ' + (cacheRoot ? psQuote('build-cache-dir=' + cacheRoot) + ' ' : '') + args
      : (cacheRoot ? 'mkdir -p ' + psQuote(cacheRoot) + ' && ' : '') + 'cd ' + psQuote(base) + ' && DEVECO_SDK_HOME=' + psQuote(sdk.sdkRoot) + ' DEVECO_CLI_STUDIO_PATH=' + psQuote(st.root) + ' ' + (cacheRoot ? psQuote('build-cache-dir=' + cacheRoot) + ' ' : '') + psQuote(hvigor.path) + ' ' + args
