...
export function apply(ctx) {
  ...
  const { ensureHdc, runHdc, listTargets, pickTarget, currentTarget, localFileExists, hdcPathRef, hdcErrorRef, diagLogRef, getPreferred, setPreferred } = hdcCore
  let shotCounter = 0
  ...
  async function hdcFallbackDeploy(args, cwd, policy) {
    ...
    const ok = installResult.ok && (!bundle || Boolean(startResult && startResult.ok))
    ...
    const text = [
      ...
      bundle
        ? `  aa start -a ${ability} -b ${bundle} -> ${startResult && startResult.ok ? 'ok' : 'FAILED: ' + ((startResult && (startResult.stderr || startResult.stdout)) || '')}`
        : '  bundleName was not readable from AppScope/app.json5; start it with hdc_app.',
    ].join('\n') + unsignedNote
    return { ok, targetId, hap, bundle, ability, installOk: installResult.ok, startOk: Boolean(startResult && startResult.ok), text }
  }
  ...
  registerTool({
    ...
    async execute(args, exec) {
      ...
      const formatted = compileOut.formatBuildProjectOutput({ stdout: result.stdout, stderr: result.stderr, exitCode: result.exitCode })
      ...
      return {
        ...
        text,
        error: ok ? '' : (cli.ok ? 'build failed' : cli.error),
      }
    },
  })
    ...
    async execute(args, exec) {
      ...
      if (action === 'list_devices') {
        ...
        if (cli.ok) {
          ...
          if (output) return { ok: result.exitCode === 0 && !mojoFatal(result), action, deviceCount: rows.length || null, lineCount: null, text: output }
        }
        const targets = await listTargets(policy)
        const text = targets.targets.length
          ? targets.targets.map((target) => `${target.id} ${target.type} ${target.state}`.trim()).join('\n')
          : 'No connected devices detected.'
        return { ok: targets.ok, action, deviceCount: targets.targets.length, lineCount: null, targets: targets.targets, text, error: targets.error || '' }
      }
