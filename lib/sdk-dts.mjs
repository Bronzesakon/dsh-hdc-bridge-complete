...
// Locate the SDK: explicit roots > DEVECO_SDK_HOME > known Studio install dirs.
// Accepts either an SDK root (…/sdk) or a DevEco Studio root (sdk/ lives under it).
export function findSdkInfo(extraRoots = []) {
  const roots = []
  for (const r of extraRoots) {
    if (!r) continue
    const resolved = pathResolve(r)
    roots.push(resolved)
    roots.push(join(resolved, 'sdk'))
  }
...
export function snippetFor(text, name, radius = 30) {
  const lines = text.split(/\r?\n/)
  const out = []
  const wanted = name && name !== 'export' ? String(name).trim() : ''
  for (let i = 0; i < lines.length; i++) {
    const match = DECL_RE.exec(lines[i])
    if (match && (!wanted || match[2] === wanted)) {
