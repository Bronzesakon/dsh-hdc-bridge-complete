...
// that chooses the safer side of the viewport, so it never becomes a draggable
// window or clips beyond the visible area.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.hdcp-panel{position:absolute;right:0;bottom:calc(100% + 8px);z-index:1000;width:360px;max-width:calc(100vw - 16px);max-height:var(--hdcp-panel-max-height,min(560px,calc(100vh - 112px)));pointer-events:auto;}',
      '.hdcp-panel-below{top:calc(100% + 8px);bottom:auto;}',
      ...
      '.hdcp-body{min-height:0;flex:1 1 auto;max-height:none;overflow:auto;padding:9px 10px;--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2);}',
    ...
    function PanelSurface(props) {
      ...
      return React.createElement('div', { className: 'hdcp-panel' + (props.below ? ' hdcp-panel-below' : ''), style: props.panelStyle, role: 'dialog', 'aria-label': '鸿蒙开发面板' },
    ...
    function HarmonyPill() {
      ...
      var layoutPair = React.useState({ below: false, maxHeight: 560 });
      ...
      React.useEffect(function () {
        ...
        function updateLayout() {
          ...
          var placeBelow = below > above;
          var available = Math.max(160, Math.min(560, Math.max(above, below)));
          setLayout({ below: placeBelow, maxHeight: available });
        }
        updateLayout();
        var frame = typeof window.requestAnimationFrame === 'function' ? window.requestAnimationFrame(updateLayout) : 0;
      ...
      return React.createElement('span', { className: 'hdcp-root', ref: rootRef },
        ...
        open ? React.createElement(PanelSurface, {
          data: data,
          below: layout.below,
          panelStyle: { '--hdcp-panel-max-height': layout.maxHeight + 'px' },
