...
function parseDf(text) {
  ...
  // Require a df-style line: 5+ columns and a percentage column like "12%".
  // Error text (e.g. "[Fail][E001005] Device not found or connected") must
  // not be parsed as storage stats.
...
export function startPanel(bridge) {
  ...
  // hdc lists every candidate target, including the host's UART serial ports
  // (COMx). Those are not HarmonyOS phones/emulators: the panel skips them so
  // a COM port can never be mistaken for the primary device.
  function isUartTarget(parts) {
    const type = String(parts[1] || '').toLowerCase()
    return type.includes('uart') || /^com\d+$/i.test(String(parts[0] || ''))
  }
  ...
  async function listTargets() {
    ...
    if (r.ok) {
      for (const raw of r.stdout.split(String.fromCharCode(13)).join('').split(String.fromCharCode(10))) {
        ...
        if (parts.length >= 2 && parts[0] !== '[Empty]' && !isUartTarget(parts)) targets.push({ id: parts[0], type: parts[1] || '', state: parts[2] || '', addr: parts[3] || '' })
      }
    }
